 ...
 */
class cachestore_redissentinel extends store implements
    ...
    lockable_cache_interface
{
    ...
    protected static function get_time(): int {
        global $CFG;
        if (PHPUNIT_TEST && !empty($CFG->phpunit_cachestore_redissentinel_time)) {
            return $CFG->phpunit_cachestore_redissentinel_time;
        }
        return time();
    }
    ...
    public static function set_phpunit_time(int $time = 0): void {
        ...
        if ($time) {
            $CFG->phpunit_cachestore_redissentinel_time = $time;
        } else {
            unset($CFG->phpunit_cachestore_redissentinel_time);
        }
    }
    ...
    public static function initialise_test_instance(definition $definition) {
        ...
        $config = get_config('cachestore_redissentinel');
        ...
        $cache = new cachestore_redissentinel('Redis test', $configuration);
    ...
    public static function config_get_serializer_options() {
        $options = array(
            Redis::SERIALIZER_PHP => get_string('serializer_php', 'cachestore_redissentinel')
        );
    ...
    public static function config_get_compressor_options() {
        $arr = [
            self::COMPRESSOR_NONE     => get_string('compressor_none', 'cachestore_redissentinel'),
            self::COMPRESSOR_PHP_GZIP => get_string('compressor_php_gzip', 'cachestore_redissentinel'),
        ];
