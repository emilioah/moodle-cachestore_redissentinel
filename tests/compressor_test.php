...
final class compressor_test extends \advanced_testcase {
    /** @var null|\cachestore_redissentinel */
    protected ?cachestore_redissentinel $store = null;
     ...
     * @return cachestore_redissentinel
     */
    public function create_store($compressor, $serializer) {
        /** @var definition $definition */
        $definition = definition::load_adhoc(store::MODE_APPLICATION, 'cachestore_redissentinel', 'phpunit_test');
        $config = cachestore_redissentinel::unit_test_configuration();
        $config['compressor'] = $compressor;
        $config['serializer'] = $serializer;
        $store = new cachestore_redissentinel('Test', $config);
    ...
    public function test_it_can_miss_one(): void {
        $store = $this->create_store(cachestore_redissentinel::COMPRESSOR_PHP_GZIP, \Redis::SERIALIZER_PHP);
    ...
    public static function provider_for_tests_setget(): array {
        if (!cachestore_redissentinel::are_requirements_met()) {
        ...
        $data = [
            ['none, none',
                \Redis::SERIALIZER_NONE, cachestore_redissentinel::COMPRESSOR_NONE,
                'value1', 'value2'],
            ['none, gzip',
                \Redis::SERIALIZER_NONE, cachestore_redissentinel::COMPRESSOR_PHP_GZIP,
                gzencode('value1'), gzencode('value2')],
            ['php, none',
                \Redis::SERIALIZER_PHP, cachestore_redissentinel::COMPRESSOR_NONE,
                serialize('value1'), serialize('value2')],
            ['php, gzip',
                \Redis::SERIALIZER_PHP, cachestore_redissentinel::COMPRESSOR_PHP_GZIP,
        ...
        if (defined('Redis::SERIALIZER_IGBINARY')) {
            $data[] = [
                'igbinary, none',
                    \Redis::SERIALIZER_IGBINARY, cachestore_redissentinel::COMPRESSOR_NONE,
                    igbinary_serialize('value1'), igbinary_serialize('value2'),
            ];
            $data[] = [
                'igbinary, gzip',
                    \Redis::SERIALIZER_IGBINARY, cachestore_redissentinel::COMPRESSOR_PHP_GZIP,
        ...
        if (extension_loaded('zstd')) {
            $data[] = [
                'none, zstd',
                \Redis::SERIALIZER_NONE, cachestore_redissentinel::COMPRESSOR_PHP_ZSTD,
                zstd_compress('value1'), zstd_compress('value2'),
            ];
            $data[] = [
                'php, zstd',
                \Redis::SERIALIZER_PHP, cachestore_redissentinel::COMPRESSOR_PHP_ZSTD,
            ...
            if (defined('\Redis::SERIALIZER_IGBINARY')) {
                $data[] = [
                    'igbinary, zstd',
                    \Redis::SERIALIZER_IGBINARY, cachestore_redissentinel::COMPRESSOR_PHP_ZSTD,
    ...
    public function test_it_can_use_getset($name, $serializer, $compressor, $rawexpected1, $rawexpected2): void {
        ...
        $rawstore = $this->create_store(cachestore_redissentinel::COMPRESSOR_NONE, \Redis::SERIALIZER_NONE);
